    ...
    if(isset($_POST['global_token']) && $_POST['global_token'] == $_SESSION['global_token']){
        switch ($_POST['action']) {
            ...
            case 'update':
                ...
                $productController = new ProductsController();
                echo ($name.$slug.$description.$features.$id);
...
class ProductsController {
    ...
    public function createProduct($name, $slug, $description, $features, $brand_id) {
        ...
        CURLOPT_POSTFIELDS => array('name' => $name,'slug' => $slug,'description' => $description, 'features' => $features,'brand_id' => $brand_id,'cover'=> new CURLFILE($_FILES['cover']['tmp_name'])),
        CURLOPT_HTTPHEADER => array(
            'Authorization: Bearer '.$_SESSION['token']
        ),
    ));
    ...
    public function updateProduct($name, $slug, $description, $features, $brand_id, $id) {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => array('name'=> $name,'slug' => $slug,'description' => $description,'features' => $features,'brand_id' => $brand_id,'id' => $id),
            CURLOPT_HTTPHEADER => array(
                'Authorization: Bearer '.$_SESSION['token']
            ),
          ));
